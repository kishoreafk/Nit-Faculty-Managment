...
const Dashboard = () => {
  ...
  const formatActivity = useCallback((data) => {
    const activities = [];
    
    if (faculty?.role === 'admin') {
      data.recentLeaves?.forEach(leave => {
        const isApproved = leave.actionStatus === 'Approved';
        const isRejected = leave.actionStatus === 'Rejected';
        const isPending = leave.actionStatus === 'Pending';
        
        activities.push({
          id: `leave-${leave.id}`,
          title: isPending ? 'Leave Request Submitted' : isApproved ? 'Leave Request Approved' : 'Leave Request Rejected',
          description: leave.activityType || `${leave.facultyName} - ${leave.leaveType}`,
          time: new Date(leave.activityDate || leave.createdAt).toLocaleDateString(),
          icon: isPending ? CalendarIcon : isApproved ? CheckCircleIcon : ExclamationTriangleIcon,
          color: isPending ? 'text-blue-500' : isApproved ? 'text-green-500' : 'text-red-500',
          bgColor: isPending ? 'bg-blue-50' : isApproved ? 'bg-green-50' : 'bg-red-50'
        });
      });
      
      data.recentProducts?.forEach(product => {
        const isApproved = product.actionStatus === 'Approved';
        const isRejected = product.actionStatus === 'Rejected';
        const isPending = product.actionStatus === 'Pending';
        
        activities.push({
          id: `product-${product.id}`,
          title: isPending ? 'Product Request Submitted' : isApproved ? 'Product Request Approved' : 'Product Request Rejected',
          description: product.activityType || `${product.facultyName} - ${product.productName || product.product_name}`,
          time: new Date(product.activityDate || product.requested_at).toLocaleDateString(),
          icon: isPending ? DocumentTextIcon : isApproved ? CheckCircleIcon : ExclamationTriangleIcon,
          color: isPending ? 'text-purple-500' : isApproved ? 'text-green-500' : 'text-red-500',
          bgColor: isPending ? 'bg-purple-50' : isApproved ? 'bg-green-50' : 'bg-red-50'
        });
      });
    } else {
      data.recentLeaves?.forEach(leave => {
    ...
    // Sort all activities by time (most recent first) and return top 5
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      console.log('📊 Dashboard activity response:', activityResponse.data);
  ...
  useEffect(() => {
    ...
    const handleRefresh = (event) => {
      console.log('🔔 Dashboard received refresh event:', event.type);
      fetchDashboardData();
    };
